...
Fixes: Sidebar Reversion, English-Only News, Tab Layout Stability.
"""
...
# --- UI STYLING (REVERTED TO WHITE SIDEBAR) ---
...
if app_mode == "Executive Summary":
    ...
    with c2:
        score = 75 if info.get("sector") in ["Technology", "Industrials"] else 60
    ...
    if NEWS_API_KEY:
        with st.spinner("Searching English sources..."):
            # Added language=en filter to the API call
            url = f"https://newsapi.org/v2/everything?q={name}&language=en&sortBy=publishedAt&apiKey={NEWS_API_KEY}"
    ...
    st.subheader("✍️ AI Outreach (English)")
    if GEMINI_API_KEY:
