import streamlit as st
import yfinance as yf
import pandas as pd
import random
import os
import requests
import google.generativeai as genai
import plotly.graph_objects as go
import plotly.express as px
import base64
from datetime import datetime, timedelta
from dotenv import load_dotenv

# --- INITIALIZATION ---
load_dotenv()
NEWS_API_KEY = os.getenv("NEWS_API_KEY")
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")

if GEMINI_API_KEY:
    genai.configure(api_key=GEMINI_API_KEY)

# --- UI CONFIGURATION & BRANDING ---
st.set_page_config(page_title="Edge Impulse | Sales Command Center", layout="wide")

st.markdown("""
    <style>
    .stApp { background-color: #ffffff; }
    
    /* Sidebar: Professional Off-White */
    section[data-testid="stSidebar"] {
        background-color: #f8f9fb;
        border-right: 1px solid #e6e9ef;
    }
    section[data-testid="stSidebar"] .stMarkdown, section[data-testid="stSidebar"] label {
        color: #31333f !important;
        font-weight: 600;
    }

    /* Score & Status Styling */
    .score-container {
        text-align: center; padding: 25px; background: white; 
        border: 1px solid #e6e9ef; border-radius: 15px;
        box-shadow: 0px 4px 10px rgba(0,0,0,0.03);
    }
    
    /* Branded Recommendation Bubble */
    .recommend-bubble {
        padding: 8px 18px; border-radius: 25px; font-weight: bold;
        display: inline-block; color: white; font-size: 0.85em;
        text-transform: uppercase;
    }
    .buy { background-color: #28a745; }
    .sell { background-color: #dc3545; }
    .hold { background-color: #ffc107; color: #000; }
    
    /* ROI & Intelligence Boxes */
    .roi-card {
        padding: 25px; background-color: #f0f7ff;
        border-radius: 12px; border-left: 5px solid #007bff;
        margin-bottom: 20px;
    }
    
    .report-box { 
        padding: 25px; background-color: #f8f9fb; 
        border-radius: 15px; border-left: 5px solid #007bff;
        margin-bottom: 20px;
    }

    /* Buttons */
    .stButton>button {
        border-radius: 8px;
        font-weight: 600;
    }
    </style>
    """, unsafe_allow_html=True)

# --- HELPER FUNCTIONS ---
def get_recommendation_label(info):
    rec = info.get('recommendationKey', 'none').lower()
    if 'buy' in rec: return "BUY", "buy"
    if 'sell' in rec: return "SELL", "sell"
    return "HOLD", "hold"

def download_link(content, filename, text):
    b64 = base64.b64encode(content.encode()).decode()
    return f'<a href="data:file/txt;base64,{b64}" download="{filename}" style="text-decoration:none;"><button style="background-color:#007bff;color:white;border:none;padding:10px 20px;border-radius:8px;cursor:pointer;">{text}</button></a>'

# --- SIDEBAR NAVIGATION ---
with st.sidebar:
    if os.path.exists("logo.png"):
        st.image("logo.png", use_container_width=True)
    else:
        st.title("EDGE IMPULSE")
    
    st.divider()
    
    app_mode = st.radio("Intelligence Suite", 
                        ["🏠 Executive Summary", 
                         "📅 Weekly News Digest",
                         "🏆 Account Leaderboard", 
                         "🕸️ Account Map", 
                         "💰 ROI Calculator", 
                         "📄 Deep Signal (10-K)",
                         "⚔️ Competitive Battlecard", 
                         "✍️ Outreach & Export"])
    
    st.divider()
    
    if app_mode == "🏆 Account Leaderboard":
        ticker_input = st.text_input("Enter Tickers (comma separated):", "F, GM, TSLA, TM").upper()
    else:
        ticker_input = st.text_input("Active Account Ticker:", "DE").upper()
    
    target_persona = st.selectbox("Persona:", ["VP Engineering", "CTO", "Head of Manufacturing", "Director Innovation"])
    sender_name = st.text_input("Your Name:", "Kush")

# --- MAIN APP LOGIC ---
if ticker_input:
    try:
        # ---------------------------------------------------------
        # SCREEN: ACCOUNT LEADERBOARD
        # ---------------------------------------------------------
        if app_mode == "🏆 Account Leaderboard":
            st.title("Strategic Portfolio Prioritization")
            st.write("Comparing accounts based on market momentum and Edge AI fit.")
            
            tickers = [t.strip() for t in ticker_input.split(",")]
            comparison_data = []
            
            with st.spinner("Processing portfolio data..."):
                for t in tickers:
                    try:
                        s = yf.Ticker(t)
                        inf = s.info
                        score = random.randint(75, 94)
                        if "Automotive" in inf.get('industry', ''): score += 5
                        comparison_data.append({
                            "Ticker": t, "Company": inf.get('shortName', t),
                            "Industry": inf.get('industry'), "Fit Score": score,
                            "Market Cap ($B)": inf.get('marketCap',0)//10**9
                        })
                    except: continue
            
            if comparison_data:
                df = pd.DataFrame(comparison_data)
                fig = px.bar(df, x="Company", y="Fit Score", color="Fit Score", color_continuous_scale="Blues")
                st.plotly_chart(fig, use_container_width=True)
                st.dataframe(df.sort_values(by="Fit Score", ascending=False), use_container_width=True)
            else:
                st.warning("Please enter valid stock tickers.")

        # ---------------------------------------------------------
        # SINGLE ACCOUNT SCREENS
        # ---------------------------------------------------------
        else:
            stock = yf.Ticker(ticker_input)
            info = stock.info
            name = info.get('shortName', ticker_input)
            
            # SHARED HEADER
            col_h1, col_h2 = st.columns([3, 1])
            with col_h1:
                st.title(f"{name} ({ticker_input})")
                st.markdown(f"**Sector:** {info.get('sector')} | **Industry:** {info.get('industry')}")
            with col_h2:
                price = info.get('currentPrice', 0.0)
                st.metric("Price", f"${price}", f"{info.get('regularMarketChangePercent', 0):.2f}%")
                label, css = get_recommendation_label(info)
                st.markdown(f'<div class="recommend-bubble {css}">Analyst View: {label}</div>', unsafe_allow_html=True)
            st.divider()

            if app_mode == "🏠 Executive Summary":
                col_l, col_r = st.columns([2, 1])
                with col_l:
                    st.subheader("Account Briefing")
                    st.write(info.get('longBusinessSummary', '')[:800] + "...")
                with col_r:
                    score = random.randint(85, 99)
                    st.markdown(f"""
                        <div class="score-container">
                            <p style="font-weight:bold; color:#6c757d; margin:0;">OVERALL FIT SCORE</p>
                            <h1 style="color:#007bff; font-size:75px; margin:0;">{score}</h1>
                            <p style="color:green; font-weight:bold;">🔥 HIGH PRIORITY LEAD</p>
                        </div>
                    """, unsafe_allow_html=True)
                    st.info(f"**AI Insight:** {name} is aggressively modernizing their {info.get('industry')} infrastructure. This is a prime opportunity for Edge Impulse.")

            elif app_mode == "📅 Weekly News Digest":
                st.subheader("7-Day Intelligence Synthesis")
                if st.button("Generate Strategic Takeaways"):
                    with st.spinner("Synthesizing weekly headlines..."):
                        seven_days_ago = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d')
                        q = f'"{name}" AND (AI OR technology OR hardware OR strategy)'
                        url = f"https://newsapi.org/v2/everything?q={q}&from={seven_days_ago}&sortBy=relevancy&apiKey={NEWS_API_KEY}"
                        articles = requests.get(url).json().get('articles', [])
                        
                        if articles:
                            context = "\n".join([f"- {a['title']}" for a in articles[:8]])
                            model = genai.GenerativeModel('gemini-2.5-flash')
                            res = model.generate_content(f"Act as a Sales Research Assistant. Based on this news for {name}, provide: 1. Top 3 Takeaways, 2. The Edge Impulse Angle, 3. A punchy email icebreaker line. News: \n{context}")
                            st.markdown(f'<div class="report-box">{res.text}</div>', unsafe_allow_html=True)
                        else: st.info("No major strategic news found in the last 7 days.")

            elif app_mode == "🕸️ Account Map":
                st.subheader("Predicted Stakeholder Map")
                if st.button("Generate Strategic Org Chart"):
                    with st.spinner("Mapping the organization..."):
                        model = genai.GenerativeModel('gemini-2.5-flash')
                        res = model.generate_content(f"Identify the likely Decision Making Unit for an Edge AI deal at {name}. Include roles like CTO, VP Eng, and influencers. Mention 'pain points' for each role.")
                        st.markdown(res.text)
                st.divider()
                st.subheader("🔍 LinkedIn Discovery")
                st.code(f'site:linkedin.com/in/ "{target_persona}" "{name}"', language="text")

            elif app_mode == "💰 ROI Calculator":
                st.subheader("Business Case: Moving Cloud Compute to the Edge")
                col_in, col_out = st.columns([1, 1])
                with col_in:
                    devices = st.slider("Number of Devices", 1000, 100000, 10000)
                    data_mb = st.slider("Data Transfer per Day (MB)", 1, 100, 20)
                    cloud_cost = st.slider("Cloud Ingress Cost ($/GB)", 0.05, 0.25, 0.12)
                    monthly_cloud = (devices * data_mb * 30 / 1024) * cloud_cost
                    annual_savings = monthly_cloud * 0.88 * 12 
                with col_out:
                    st.markdown(f'<div class="roi-card"><h4>Potential Annual Savings</h4><h1 style="color:#28a745;">${annual_savings:,.0f}</h1><p>Based on displacing 88% of cloud processing costs.</p></div>', unsafe_allow_html=True)
                    fig = go.Figure(data=[go.Bar(x=['Cloud-Only', 'Edge Impulse'], y=[monthly_cloud*12, (monthly_cloud*12)-annual_savings], marker_color=['#6c757d','#007bff'])])
                    fig.update_layout(title="Annual Infrastructure Cost Comparison", height=350)
                    st.plotly_chart(fig)

            elif app_mode == "📄 Deep Signal (10-K)":
                st.subheader("AI Filing Audit")
                if st.button("Analyze Business Strategy"):
                    with st.spinner("Parsing latest reports..."):
                        model = genai.GenerativeModel('gemini-2.5-flash')
                        res = model.generate_content(f"Analyze the business model of {name}. What are 3 strategic reasons they need to move towards on-device/Edge AI to save costs or improve product performance?")
                        st.markdown(res.text)

            elif app_mode == "⚔️ Competitive Battlecard":
                st.subheader("Competitive Positioning")
                if st.button("Generate Competitive Playbook"):
                    with st.spinner("Analyzing rivals..."):
                        model = genai.GenerativeModel('gemini-2.5-flash')
                        res = model.generate_content(f"Create a sales battlecard for selling Edge Impulse to {name}. Compare Edge Impulse vs Big Cloud vs DIY. Highlight the 'Knockout' points.")
                        st.markdown(res.text)

            elif app_mode == "✍️ Outreach & Export":
                st.subheader("Outreach Engine & CRM Bridge")
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("Draft Cold Email"):
                        with st.spinner("Writing email..."):
                            model = genai.GenerativeModel('gemini-2.5-flash')
                            email = model.generate_content(f"Draft a punchy, ROI-focused cold email to {target_persona} at {name}. Focus on Edge AI value. Sign: {sender_name}.").text
                            st.session_state['current_email'] = email
                    if 'current_email' in st.session_state:
                        st.text_area("Drafted Email Content", st.session_state['current_email'], height=300)
                with col2:
                    st.write("### Internal Data Sync")
                    csv = f"Ticker,Company,Persona,Date,Author\n{ticker_input},{name},{target_persona},{datetime.now().date()},{sender_name}"
                    st.download_button("📥 Download Lead CSV", csv, f"{ticker_input}_lead.csv")
                    brief = f"# Account Briefing: {name}\n\nGenerated by: {sender_name}\n\n## Business Context\n{info.get('longBusinessSummary')[:1000]}..."
                    st.markdown(download_link(brief, f"{ticker_input}_brief.md", "📄 Export Markdown Brief"), unsafe_allow_html=True)

            # ACCOUNT CONCIERGE
            st.divider()
            st.subheader("💬 Account Concierge")
            q = st.chat_input(f"Ask me anything about {name}'s technology strategy...")
            if q:
                with st.chat_message("assistant"):
                    model = genai.GenerativeModel('gemini-2.5-flash')
                    ans = model.generate_content(f"Context: {info.get('longBusinessSummary')}. Question: {q}")
                    st.write(ans.text)

    except Exception as e:
        st.error(f"Account Loading Error: {e}")
